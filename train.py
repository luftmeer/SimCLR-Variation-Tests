...
def train(model, optimizer, loss_fn, train_loader, local_rank, global_rank, monitor, epoch, args):
    ...
    for step, (augmentations, labels) in tqdm.tqdm(enumerate(train_loader), desc="Training", total=len(train_loader)):
        ...
        for i, h in enumerate(hs):
            all_embeddings[i].append(h.detach().cpu())
        
        for i, z in enumerate(zs):
            all_projections[i].append(z.detach().cpu())
        
        for comb_nr, (z_i, z_j) in enumerate(combinations(zs, 2)):
...
def main(args):
    ...
    monitor = TrainingMonitor(
        ...
        plot_every=100,
        batch_size=args.batch_size * args.ga_count if args.ga else 1,
        maxlen=500,
        enabled=True,
        rank=0,
    ...
    for epoch in range(start_epoch, args.epochs):        
        ...
        loss_epoch, sim, all_embeddings, all_projections, all_labels = train(model, optimizer, loss_fn, train_loader, device, 0, monitor, epoch, args)
